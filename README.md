...
- **Real-world Integration**: Connected to Directus CMS for realistic data fetching scenarios
...
4. **SSR (Server-Side Rendering)**
   - Rendered on each request with caching
...
- Docker (for Redis)
- Yarn or npm
- Directus instance (optional, for data fetching)
...
DIRECTUS_HOST=http://your-directus-host
...
const data = await getPromotions(60); // Revalidate every 60 seconds
```
...
```typescript
// app/page.tsx
const data = await getPromotions(undefined); // cache: 'no-store'
```
- Fresh data on every request
- No caching applied
...
| `DIRECTUS_HOST` | No** | Directus API base URL | `http://localhost:8055` |
| `DIRECTUS_TOKEN` | No** | Directus authentication token | `your-token-here` |

\* Either `REDIS_URL` or `KV_URL` required for Redis caching. Falls back to in-memory if neither is set.
\** Required only if using Directus integration. Will show config error without these.
...
- **[Directus](https://directus.io/)** - Headless CMS for data source
